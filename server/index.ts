...
const isArchivedStatus = (value: unknown) =>
    typeof value === 'string' && ARCHIVED_STATUSES.has(normalizeLifecycleStatus(value));

const isCanceledBookingStatus = (value: unknown) =>
    typeof value === 'string' && CANCELED_BOOKING_STATUSES.has(normalizeLifecycleStatus(value));

const toArchiveStatus = (value: unknown, fallback = 'archived') => {
...
type EmailLanguage = 'el' | 'en';

const normalizeEmailLanguage = (value: unknown): EmailLanguage => {
    if (typeof value !== 'string') {
        return 'en';
    }
...
const getLocalizedTemplateContent = (
    template: {
...
const getMappedTemplates = async (purposes: string[]) => {
    ...
    return new Map(
        (data || []).map((row: any) => [row.purpose, row.email_templates as any])
    );
};
...
const sortProductCategories = (productCategories: any[] = []) =>
    [...productCategories]
        .map((productCategory) => ({
            ...productCategory,
...
const flattenProductCategories = (productCategories: any[] = []) =>
    sortProductCategories(productCategories).flatMap((productCategory) =>
        Array.isArray(productCategory?.products) ? productCategory.products : []
    );

const attachProductCategoryData = (event: any) => {
...
const isShiftCapacityBookingError = (error: unknown) => {
    const candidate = error as { code?: string; message?: string; details?: string; hint?: string } | null;
    if (!candidate) {
        return false;
    }
    ...
    const looksLikeCapacityError = (
        searchableText.includes('capacity') ||
...
const isInvalidBookingProductError = (error: unknown) => {
    const candidate = error as { code?: string; message?: string; details?: string; hint?: string } | null;
    if (!candidate) {
        return false;
    }
    ...
    return candidate.code === '23503' && (
        searchableText.includes('booking_products') ||
...
const summarizeBookingProductsForLogs = (products: any[]) =>
...
// Validation Helper
const isValidISODatetime = (iso: string): boolean => {
...
const cancelPageHtml = (bookingName: string, eventTitle: string, bookingId: string, token: string) => `
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
...
const cancelErrorHtml = (message: string) => `
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
...
const getShiftCancelledTemplate = async () => {
    ...
    return purposeData?.email_templates as any;
};

const sendShiftCancelledEmails = async ({
    event,
    shift,
    bookings,
    template,
}: {
    event: { id: number | string; title: string; location_name?: string | null };
    shift: { id: number | string; start_time: string };
    bookings: any[];
    template?: any;
}) => {
...
const archiveShiftAndNotify = async (
...
app.get('/api/cancel-booking/:id', async (req, res) => {
    ...
    try {
        ...
        const eventTitle = (booking.events as any)?.title || 'your experience';
      ...
      `)
        ...
        if (data) {
            const visibleEvents = data
                .filter((event: any) => !isArchivedStatus(event.status))
                .map((event: any) => {
                    if (event.shifts) {
                        event.shifts = event.shifts.filter(
                            (shift: any) => !isArchivedStatus(shift.status) && shift.is_active !== false
                        );
                    }
...
app.get('/api/events/:slug', async (req, res) => {
    try {
      ...
      `)
        ...
        if (data?.shifts) {
            data.shifts = data.shifts.filter(
                (shift: any) => !isArchivedStatus(shift.status) && shift.is_active !== false
            );
        }
        ...
        if (process.env.RESEND_API_KEY) {
            ...
            if (adminRecipients.length > 0) {
                ...
                try {
                    ...
                    await supabase.from('email_logs').insert(
                        adminRecipients.map((recipientEmail) => ({
                            booking_id: null,
                            shift_id: null,
                            event_id: null,
...
app.post('/api/bookings', bookingRateLimit, async (req, res) => {
    ...
    let observedCapacity: number | null = null;
    let observedCurrentTotal: number | null = null;
    let observedRemainingSpots: number | null = null;
    ...
    try {
        ...
        const currentTotal = (existingBookings || []).reduce(
            (sum, booking) => isCanceledBookingStatus(booking.status) ? sum : sum + (booking.number_of_people || 0),
            0
        );
        ...
        if (eventProductCategoriesError) throw eventProductCategoriesError;

        const populatedProductCategories = sortProductCategories(eventProductCategories || [])
            .filter((productCategory: any) => (productCategory.products?.length || 0) > 0);

        const hasInvalidProductPayload = selectedProducts.some((product: any) =>
            !product ||
        ...
        if (requiredProductSelectionCount === 0) {
            ...
            const totalProductQuantity = selectedProducts.reduce((sum: number, product: any) => sum + product.quantity, 0);
            ...
            const productCategoryByProductId = new Map<string, number>();
            ...
            const selectedCategoryQuantities = new Map<number, number>();
            ...
            for (const product of selectedProducts) {
                ...
                selectedCategoryQuantities.set(
                    productCategoryId,
                    (selectedCategoryQuantities.get(productCategoryId) || 0) + product.quantity
                );
            }

            const hasIncompleteCategorySelections = populatedProductCategories.some(
                (productCategory: any) => (selectedCategoryQuantities.get(Number(productCategory.id)) || 0) !== normalizedPeople
            );
            ...
            productTotal = selectedProducts.reduce(
                (sum: number, product: any) => sum + (product.quantity * (productPriceById.get(String(product.product_id)) || 0)),
                0
            );
        }
        ...
        if (selectedProducts.length > 0) {
            ...
            const bookingProducts = selectedProducts.map((p: any) => ({
                booking_id: booking.id,
            ...
            (async () => {
                try {
                    ...
                    const formatEmail = (
                        ...
                        bookingId: number,
                        peopleCount: number,
                        billAmount: number | string | null | undefined,
                        isHtml = false
                    ) => {
                    ...
                    const activeShiftBookings = (allShiftBookings || []).filter(
                        (shiftBooking: any) => !isCanceledBookingStatus(shiftBooking.status)
                    );

                    const emailTotalPeople = activeShiftBookings.reduce(
                        (sum, shiftBooking) => sum + (Number(shiftBooking.number_of_people) || 0),
                        0
                    );
                    ...
                    if (isGoalMetNow && shift && !shift.is_confirmed) {
                        ...
                        if (paymentInvitationTemplate && activeShiftBookings.length > 0) {
                            const previousBookings = activeShiftBookings.filter((shiftBooking: any) => shiftBooking.id !== booking.id);
...
app.get('/api/admin/events', requireAdmin, async (req, res) => {
    try {
      ...
      `)
        ...
        if (data) {
            res.json(data.map((event: any) => attachProductCategoryData(event)));
            return;
        }
...
app.post('/api/admin/validate-event', requireAdmin, (req, res) => {
    ...
    if (!data.event_date || !isValidISODatetime(data.event_date)) errors.push('Valid event date and time are required');
    if (!data.booking_deadline || !isValidISODatetime(data.booking_deadline)) errors.push('Valid booking deadline is required');
    if (!data.payment_deadline || !isValidISODatetime(data.payment_deadline)) errors.push('Valid payment deadline is required');
...
app.post('/api/admin/events', requireAdmin, async (req, res) => {
    ...
    try {
        ...
        if (safeImageUrls.length > 0) {
            const imageRecords = safeImageUrls.map((url: string, index: number) => ({
                event_id: event.id,
                image_url: url,
                is_cover: index === 0
            }));
        ...
        if (safeShifts.length > 0) {
            const shiftRecords = safeShifts.map((s: any) => ({
                event_id: event.id,
        ...
        const normalizedProductCategories = (
            ...
            .filter((productCategory: any) => productCategory.name);
...
app.delete('/api/admin/event-images/:id', requireAdmin, async (req, res) => {
    try {
        ...
        if (remainingImagesError) throw remainingImagesError;
        ...
        if (imageRecord.is_cover) {
            ...
            if (resetRemainingCoverError) throw resetRemainingCoverError;
            ...
            if (setNextCoverError) throw setNextCoverError;
...
app.delete('/api/admin/product-categories/:id', requireAdmin, async (req, res) => {
    try {
        ...
        const productIds = (categoryProducts || []).map((product: any) => product.id);
...
app.post('/api/admin/email-templates', requireAdmin, async (req, res) => {
    ...
    if (
        !isNonEmptyString(safeTemplate.subject_eng) ||
